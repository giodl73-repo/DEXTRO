# Wave F7: Research Experiments

**Date**: TBD (Planned)
**Focus**: Alternative representation systems and redistricting experiments for academic research
**Status**: 📋 PLANNED
**Priority**: RESEARCH (deferred until after core research)
**Estimated Duration**: 6-8 weeks
**Enhancements**: 22, 23, 24, 25, 26, 27, 28
**Phases**:
- Phase 1: Enhancement 22 - National Districts (📋 PLANNED)
- Phase 2: Enhancement 23 - County Representation (📋 PLANNED)
- Phase 3: Enhancement 24 - Party Allocation (📋 PLANNED)
- Phase 4: Enhancement 25 - Committee System (📋 PLANNED)
- Phase 5: Enhancement 26 - Demographic Districts (📋 PLANNED)
- Phase 6: Enhancement 27 - Electoral Reform (📋 PLANNED)
- Phase 7: Enhancement 28 - Multi-Member (📋 PLANNED)

---

## Goals

1. Explore alternative representation systems beyond current redistricting
2. Generate research ideas for future papers and conference presentations
3. Investigate proportional representation and non-geographic districting
4. Compare different democratic representation mechanisms

---

## Success Metrics

| Metric | Target | Notes |
|--------|--------|-------|
| Alternative systems | 7 variants | Diverse representation approaches |
| Research papers | 2-3 | Conference/journal submissions |
| Novel insights | Multiple | New perspectives on representation |

---

## Phases

### Phase 1: Enhancement 22 - National Redistricting (No State Boundaries)
**Status**: 📋 PLANNED
**Priority**: RESEARCH
**Estimated Effort**: 1-2 weeks

Create 435 congressional districts nationwide without respecting state boundaries.

**Research Question**: How much better could district compactness be if we ignored state boundaries? What would "optimal" national redistricting look like?

### Phase 2: Enhancement 23 - Direct County Representation
**Status**: 📋 PLANNED
**Priority**: RESEARCH
**Estimated Effort**: 1 week

Allocate representatives directly to counties based on population.

**Research Question**: What if we used existing political boundaries (counties) instead of creating new districts? How would representation change?

### Phase 3: Enhancement 24 - Party-Based District Allocation
**Status**: 📋 PLANNED
**Priority**: RESEARCH
**Estimated Effort**: 1 week

Create districts to match statewide partisan vote shares (proportional representation).

**Research Question**: What if districts were drawn to match statewide party preferences? How different from current/algorithmic districts?

### Phase 4: Enhancement 25 - Committee-Based Representation
**Status**: 📋 PLANNED
**Priority**: RESEARCH
**Estimated Effort**: 1 week

Design representation system based on policy committees instead of geography.

**Research Question**: What if representation was organized by policy interests rather than location? Could voters choose committee representation?

### Phase 5: Enhancement 26 - Demographic Similarity Districts
**Status**: 📋 PLANNED
**Priority**: RESEARCH
**Estimated Effort**: 1-2 weeks

Create districts by clustering demographically similar census tracts.

**Research Question**: What if districts grouped people by demographic characteristics instead of geography? How would this change representation?

### Phase 6: Enhancement 27 - Electoral College County-Based Reform
**Status**: 📋 PLANNED
**Priority**: RESEARCH
**Estimated Effort**: 1 week

Reform Electoral College by allocating votes at county level instead of state level.

**Research Question**: How would presidential elections change if Electoral College votes were allocated by county rather than winner-take-all by state?

### Phase 7: Enhancement 28 - Multi-Member Districts
**Status**: 📋 PLANNED
**Priority**: RESEARCH
**Estimated Effort**: 1-2 weeks

Create larger districts with multiple representatives (proportional representation).

**Research Question**: What if we had fewer, larger districts with multiple representatives elected proportionally? How would this affect representation and gerrymandering?

---

## Dependencies

**Prerequisites**:
- WAVE-F3 (Research infrastructure) - Need research narrative framework
- Multiple completed research papers - Experiments should come after core research

**Blocking Issues**: None

---

## Risks & Mitigation

| Risk | Impact | Mitigation |
|------|--------|------------|
| Experiments too speculative | Medium | Frame as thought experiments, not proposals |
| Research distraction | High | DEFER until after core papers published |
| Public misunderstanding | Medium | Clear communication: academic research, not advocacy |

---

## Related Enhancements

- [Enhancement 22](../enhancements/22_national_redistricting.md) - National Redistricting (planned)
- [Enhancement 23](../enhancements/23_county_representation.md) - Direct County Representation (planned)
- [Enhancement 24](../enhancements/24_party_based.md) - Party-Based Allocation (planned)
- [Enhancement 25](../enhancements/25_committee_based.md) - Committee-Based Representation (planned)
- [Enhancement 26](../enhancements/26_demographic_similarity.md) - Demographic Similarity Districts (planned)
- [Enhancement 27](../enhancements/27_electoral_college.md) - Electoral College Reform (planned)
- [Enhancement 28](../enhancements/28_multi_member.md) - Multi-Member Districts (planned)

---

## Why This Wave Should Be Deferred

1. **Research Focus**: Core algorithmic redistricting research should be published first
2. **Long Duration**: 6-8 weeks is significant time investment
3. **Speculative**: Experiments may not yield publishable results
4. **Lower Impact**: Alternative systems are less directly applicable than core work
5. **Distraction Risk**: Could divert attention from high-priority research

**Recommendation**: Complete WAVE-F2, F3, F4, F5 first. Revisit when core research is published and stable.

---

**Wave F7 Summary**: Explore alternative representation systems for future research. Deferred until core redistricting research is complete and published.
