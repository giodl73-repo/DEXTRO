...
\textbf{Water-Based Adjacency:} For 30+ states with islands or water barriers (including Hawaii, Alaska, Washington, Michigan, New York, California, Florida, and Maine), we employ county-based bridge connections to ensure graph connectivity. We identify all disconnected components in the land-based adjacency graph, then connect each component to the nearest tract in the main component within the same county (using the first 5 digits of the 11-digit tract GEOID: SSCCCTTTTTT where SS = state FIPS, CCC = county FIPS). Distance is measured by centroid-to-centroid Euclidean distance after projection to state-specific coordinate reference systems (e.g., California Albers EPSG:3310). This minimal bridging approach creates only necessary water crossings while respecting county boundaries, unlike distance-threshold methods which create excessive edges or fail for distant islands (e.g., Alaska's Aleutian Islands at ~1700 km separation). Without this modification, island populations would form disconnected components, preventing contiguous district creation.
