#!/usr/bin/env python3
"""
Compute tract adjacency graphs with boundary lengths for 2010 Census data.
...
def compute_state_adjacency(state_code, tracts_dir, output_dir, water_distance_km=5.0):
    ...
    # Input file
    tracts_file = tracts_dir / f"{state_lower}_tracts_2010.parquet"
    ...
    # Output file
    output_file = output_dir / f"{state_lower}_adjacency_2010.pkl"
...
def main():
    parser = argparse.ArgumentParser(
        description='Compute tract adjacency graphs with boundary lengths for 2010 Census',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process single state
  python compute_tract_adjacencies_2010.py --states CA

  # Process all 50 states
  python compute_tract_adjacencies_2010.py

  # Adjust water distance threshold
  python compute_tract_adjacencies_2010.py --water-distance 3.0
        """
    )
    parser.add_argument(
        '--tracts-dir',
    ...
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('data/adjacency/2010'),
    ...
    if not args.tracts_dir.exists():
        ...
        print()
        print("Run these scripts first to create tract data:")
        print("  python scripts/data/census/parse_pl94171_tracts_2010.py")
        print("  python scripts/data/geography/download_tiger_tracts_2010.py")
        print("  python scripts/data/merge_tracts_with_geometries_2010.py")
        return 1
    ...
    print("=" * 70)
    print("COMPUTING TRACT ADJACENCY GRAPHS WITH BOUNDARY LENGTHS (2010)")
    print("=" * 70)
    ...
    print("Run: python scripts/pipeline/run_complete_redistricting.py --year 2010 --version v1")
