...
- [ ] **1.1** Inventory every manifest emitted today. Include at minimum: `PlanManifest` (from `redist-report::manifest`), the deposition log sidecar manifest (per Deposition spec v2.1), `narrative_manifest.json` (per Plan Comparison spec v2.1), `what_if/.../manifest.json`, and the new `reproducibility_package_manifest.json` introduced by this plan.
- [ ] **1.2** Document each field's type, required/optional, and exact derivation (e.g., `binary_sha256` is the SHA-256 of the running binary's executable file at the time of plan creation; `tiger_sha256` is the SHA-256 of the upstream .zip on Census.gov).
...
- [ ] **4.4** Add new CLI flags from the spec: `--expert-name`, `--expert-credentials`, `--expert-affiliation`, `--case-caption-file`, `--jurisdiction`, `--citation-style`, plus the new `--allow-non-strict-civic` (BD-R1, see Task 8). Allow loading from `expert.toml` if `--expert-config <path>` is set; CLI flags override file values.
