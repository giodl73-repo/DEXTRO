...
///
/// INVARIANT: adjwgt: None in -> adjwgt: None out (no || true bug).
/// Weight accumulation uses i64 to prevent overflow (PP-02).
pub fn build_coarse_graph(g: &CsrGraph, cmap: &[u32], cn: usize) -> (CsrGraph, CoarseMap) {
    let n = g.n();
    let ncon = g.ncon as usize;

    // Accumulate vertex weights in i64 (PP-02: overflow prevention)
