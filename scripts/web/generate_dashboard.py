...
def generate_dashboard(year, version, partition_mode='normal', output_dir=None, template_file=None):
    ...
    # Load VRA analysis data (only present in metis-vra runs)
    vra_data = {}
    if states_dir and states_dir.exists():
        for state_dir in sorted(states_dir.iterdir()):
            if not state_dir.is_dir():
                continue
            vra_pkl = state_dir / 'data' / 'vra_analysis.pkl'
            if vra_pkl.exists():
                try:
                    with open(vra_pkl, 'rb') as f:
                        vra_analysis = pickle.load(f)
                    vra_data[state_dir.name] = vra_analysis
                    # Also export as JSON for the download link
                    vra_json = state_dir / 'data' / 'vra_analysis.json'
                    with open(vra_json, 'w') as f:
                        json.dump(vra_analysis, f, indent=2)
                except Exception as e:
                    print(f"  Warning: Could not load VRA data for {state_dir.name}: {e}")
    if vra_data:
        print(f"  VRA data: {len(vra_data)} states")
    vra_data_js = f"const VRA_DATA = {json.dumps(vra_data, indent=8)};"
