#!/usr/bin/env python3
"""
Parse 2020 Census PL 94-171 tract-level data into unified format.

This script extracts census tract records (SUMLEV=140) from the geographic
and population files and creates CSV files matching the standard tract format.
"""
...
def parse_2020_geo_file(geo_file):
    """Parse 2020 census geographic header file for tract-level records.

    Format: Pipe-delimited with header
    Key fields:
    - SUMLEV: Summary level (140 = tract)
    - LOGRECNO: Logical record number (for joining with population file)
    - GEOID: Geographic identifier (11 digits for tracts)
...
def process_state(state_code, census_dir, output_dir, use_status=False):
    """Process one state's 2020 census tract data."""
    ...
    # Save CSV
    output_file = output_dir / f"{state_lower}_tracts_2020_population.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description='Parse 2020 Census PL 94-171 tract-level data'
    )
    parser.add_argument(
        '--census-dir',
        type=Path,
    ...
    if use_status:
        _reporter.report(f"Processing {len(states_to_process)} states (2020)")
    else:
