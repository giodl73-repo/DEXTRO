#!/usr/bin/env python3
"""
Parse 2020 Census PL 94-171 block-level data into unified format.

This script extracts census block records (SUMLEV=750) from the geographic
and population files and creates CSV files matching the standard unit format.

WARNING: Block-level data is VERY large (~11M blocks nationally). Processing
all 50 states may take 1-2 hours and produce ~2GB of CSV files.
"""
...
def parse_2020_geo_file(geo_file):
    """Parse 2020 census geographic header file for block-level records.

    Format: Pipe-delimited with header
    Key fields:
    - SUMLEV: Summary level (750 = block)
    - LOGRECNO: Logical record number (for joining with population file)
    - GEOID: Geographic identifier (15 digits for blocks: SSCCCTTTTTTBBBB)
    ...
    # Filter to block level (SUMLEV='750')
    df = df[df['SUMLEV'] == '750'].copy()
...
def process_state(state_code, census_dir, output_dir, use_status=False):
    """Process one state's 2020 census block data."""
    ...
    # Save CSV
    output_file = output_dir / f"{state_lower}_blocks_2020_population.csv"
    df.to_csv(output_file, index=False)

    log(f"Saved {len(df)} blocks to {output_file.name}")
    return True


def main():
    parser = argparse.ArgumentParser(
        description='Parse 2020 Census PL 94-171 block-level data'
    )
    parser.add_argument(
        '--census-dir',
        type=Path,
    ...
    if use_status:
        _reporter.report(f"Processing {len(states_to_process)} states (2020 blocks)")
    else:
        print(f"Processing {len(states_to_process)} states (blocks)...")
        print(f"Census directory: {args.census_dir}")
        print(f"Output directory: {args.output_dir}")
        print()
        print("WARNING: Block-level processing is memory-intensive.")
        print("Expected output size: ~2GB for all 50 states")
        print()
