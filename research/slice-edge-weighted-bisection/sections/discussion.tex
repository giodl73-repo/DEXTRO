...
The particularly strong results in gerrymandered states (Illinois +174\%, Louisiana +104\%, Texas +86\%) reveal a deeper connection between boundary minimization and partisan neutrality.
...
Boundary minimization naturally resists these manipulations: partisan advantage requires geographic distortion, and distortion increases perimeter. By optimizing perimeter \textit{without political data}, edge-weighted recursive bisection produces districts that are geometrically neutral—they cannot favor either party because they minimize the very metric that partisan gerrymandering exploits.

This is not to say our method is \textit{politically} neutral—any redistricting plan has partisan effects due to geographic sorting of voters~\cite{rodden2019geography}. Rather, it is \textit{algorithmically} neutral: compactness emerges from geometric optimization, not partisan intent.
...
\textbf{Political neutrality vs political outcomes}: While our method is algorithmically neutral (no partisan data), any redistricting plan has partisan consequences due to voter geography. Edge-weighted bisection cannot eliminate the "geography is destiny" problem inherent to single-member districts.
