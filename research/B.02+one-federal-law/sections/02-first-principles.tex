...
The adjacency constraint does not change the problem's structure; it changes
the feasible set.
The Huntington-Hill priority rule---assign the next unit to the district
with the highest priority value---still applies.
In the intrastate case, the ``unit'' being assigned is a census tract,
and the adjacency graph of the state constrains which tracts can be
assigned together.
This is what \ar{} implements: a graph-partitioning realization of the
Huntington-Hill priority rule on the state's TIGER/Line adjacency graph.
