...
\begin{theorem}[Mixing Time Lower Bound]
\label{thm:mixing}
...
and consequently:
\[
  \tmix(0.01) = O\!\left(n^2 \log n\right).
\]
\end{theorem}
...
which is tightened to $O(n^2 \log n)$ using the path congestion bound
directly.
\end{proof}
...
\begin{table}[h]
\centering
\caption{Theoretical mixing time upper bounds $O(n^2 \log n)$ and
