...
(c) Nearest-tract snap replaces the optimal $\bar{\mathbf{c}}_j$ with the closest
    ...
    is $P_j\|\mathbf{s}_j^* - \bar{\mathbf{c}}_j\|^2$, which is small when the snap
    distance is small.
    In the $\varepsilon$-convergence regime, $\|\mathbf{s}_j - \mathbf{s}_j^{\mathrm{prev}}\| < \varepsilon$
    implies the snap distance is bounded, so the additional energy is $O(\varepsilon^2)$.
