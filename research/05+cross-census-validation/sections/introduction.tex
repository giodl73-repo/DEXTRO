...
Consider a recursive bisection algorithm~\cite{dellaLibera2026recursive} that produces more compact districts in California in 2020 than in 2000. This could indicate:
\begin{itemize}
...
This paper addresses a critical methodological gap in the broader research program on algorithmic congressional redistricting via recursive bisection~\cite{della-libera-recursive-bisection,della-libera-edge-weighted,della-libera-vra-compliance}. Those foundational studies demonstrate technical feasibility, compactness optimization, and VRA compliance across single census cycles, but a fundamental validation question remains: do algorithmic results represent stable patterns that generalize across demographic change, or do they reflect contingent outcomes specific to particular census tract configurations? This work introduces the slice-based validation framework to answer that question, enabling rigorous assessment of temporal stability—a prerequisite for deploying algorithmic redistricting as persistent policy infrastructure rather than one-time experiments. Related papers in this program investigate parameter sensitivity, compare recursive to n-way partitioning, and analyze long-term demographic-geographic dynamics across multiple census decades.
