...
\begin{proof}
  \emph{Proof sketch.} The merged region has $m = n/k$ tracts on average.
  Wilson's algorithm samples a UST in $O(m \log m)$ expected time for planar
  subgraphs. For each UST, we enumerate all $m-1$ edges and check population
  balance in $O(m)$. If no balanced cut exists, resample. By the intermediate
  value theorem on the discrete cumulative population, at least one spanning
  tree cut yields a balanced split whenever the two districts have total
  population within the tolerance bound --- which holds by construction. Formally:
  any spanning tree $\mathcal{T}$ of $H$ is a connected tree on $V_H$; removing
  any single edge $e \in \mathcal{T}$ produces exactly two connected components
  $(A_e, B_e)$. As $e$ ranges over all $|V_H|-1$ edges of $\mathcal{T}$, the
  left-component populations $\{\text{pop}(A_e)\}$ traverse from near-zero
  (single leaf) to near-$\text{pop}(H)$ (all but one leaf), with adjacent edges
  in tree order differing by at most $\max_v \text{pop}(v)$. For any target
  ratio $r \in (0,1)$ and tolerance $\varepsilon \geq \max_v \text{pop}(v) /
  \text{pop}(H)$, a balanced cut edge $e^*$ always exists. Since tracts
  represent $\ll 0.5\%$ of any node's population in all three experimental
  states (NC/WI/TX), the tolerance condition is satisfied by construction.
  After at most $O(\log(1/\delta))$ resamples, a balanced cut is found with
  probability $\geq 1 - \delta$. $\square$
\end{proof}
