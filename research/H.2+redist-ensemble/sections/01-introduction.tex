...
Wilson's algorithm runs in $O(\tau_{\mathrm{cover}})$ expected time
\citep{wilson1996generating}, where $\tau_{\mathrm{cover}}$ is the
cover time of the underlying random walk.
For planar graphs---the natural setting for census-tract adjacency
graphs---Aldous~\citeyearpar{aldous1991random} established that
$\tau_{\mathrm{cover}} = O(|V|\log|V|)$; the planar bound is his
result, not Wilson's.
