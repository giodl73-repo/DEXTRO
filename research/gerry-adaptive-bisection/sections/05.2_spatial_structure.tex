...
\subsubsection{Spatial Autocorrelation: Moran's I}
...
\subsubsection{Spatial Clustering in Test States}

Table~\ref{tab:spatial-autocorrelation} shows Moran's I for our five test states:
...
\label{tab:spatial-autocorrelation}
\end{table}
...
\subsubsection{Relationship 	o Method Equivalence}
...
\textbf{Formal connection}: Recall from Theorem~\ref{thm:convergence} that method equivalence requires $\alpha \geq \alpha_{\text{crit}} \approx k / m_{\text{state}}$. However, this bound assumes worst-case spatial distribution. With high spatial autocorrelation, the effective $\alpha_{\text{crit}}$ is lower:
...
\subsubsection{Generalization Boundaries}

Our results generalize 	o states with high minority clustering but may not hold for dispersed minority populations:
...
\bottomrule
\end{tabular}
\caption{Predicted method equivalence thresholds as a function of spatial clustering (Moran's I). States with dispersed minority populations require higher $\alpha$ 	o achieve method equivalence.}
...
Figure~\ref{fig:spatial-clustering} visualizes minority clustering for Georgia:

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/georgia_minority_clustering.png}
\caption{Spatial distribution of minority VAP percentage in Georgia (2020 census). Dark regions show minority-dense tracts (> 40\% minority). Clear spatial clustering is visible in Atlanta metro, southwest Georgia (Albany corridor), and coastal regions (Savannah). This clustering produces Moran's I = 0.683 and enables method equivalence at $\alpha = 5$.}
\label{fig:spatial-clustering}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Alpha calibration}: Use spatial autocorrelation 	o calibrate edge weight factor:
    \[
    \alpha_{\text{recommended}} = \frac{2k}{m_{\text{state}} \cdot \max(I, 0.5)}
    \]
    This ensures robustness across varying spatial structures.

    \item \textbf{Litigation strategy}: In VRA cases, plaintiffs can use Moran's I 	o argue that method equivalence should hold, reducing discretion in algorithm choice. Conversely, defendants facing dispersed minorities ($I < 0.3$) can argue that method choice legitimately affects outcomes.
...
\begin{itemize}
    ...
    \item \textbf{Scale dependence}: Does clustering at tract level ($n \approx 1000$) translate 	o clustering at block group level ($n \approx 5000$) or precinct level ($n \approx 2000$)?
...
The finding generalizes 	o states with similar clustering (I > 0.6) but may require higher $\alpha$ for dispersed minority populations (I < 0.3). Practitioners should assess spatial structure before algorithm selection 	o determine appropriate edge weight calibration.
