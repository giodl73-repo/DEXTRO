...
The D/R seat counts are identical across all four algorithms for all three
states.
This is consistent with the B.8 finding that seat counts are determined
primarily by the geographic distribution of partisan voters and the
bisection tree structure, not by fine-grained boundary adjustments at the
margin~\citep{b8paper}.
SA's improvement in compactness does not shift seats because the seat
boundaries are set by the same bisection tree; SA only adjusts which tracts
fall on each side of each cut.
...
This ordering makes intuitive sense: \be{} benefits from both the best
single-seed selection \emph{and} the SA refinement would apply to each of
those seeds; SA(steps=10) on a single seed captures the refinement benefit
without the multi-seed selection benefit.
