...
This is implemented via a BFS from an interior tract in each piece.
...
\item Total: $O(\mathtt{steps\_per\_tract} \times m^2)$ for contiguity checks,
  which dominates.
\end{itemize}
For a tree with $k-1$ internal nodes averaging $m/\log k$ tracts per node,
the total SA cost per plan is $O(\mathtt{steps\_per\_tract} \times m^2 / \log k)$.
\end{proposition}

\begin{remark}
The $O(m^2)$ per-node cost appears steep, but in practice the contiguity BFS
terminates early (after visiting a connected component of typical size $O(\sqrt{m})$
for planar tract graphs), and the inner loop constant is small (single
integer comparisons).
Empirical runtimes in Section~\ref{sec:empirical} show that \sa{} with
$\mathtt{steps\_per\_tract} = 10$ is approximately $12\times$ slower than a
single \metis{} call per node, which remains practical for all 50 states.
\end{remark}
