...
Values are means over $n_{\text{runs}}=5$ independent chains ($s \in \{42,\ldots,46\}$, $T=2{,}000$ steps). Standard errors on autocorrelation estimates are $\pm 0.03$ for multi-scale and $\pm 0.04$ for standard \recom{} (bootstrap, 1,000 resamples). The 43\% reduction in lag-100 autocorrelation is robust across all 5 seed pairs ($p < 0.01$ by paired $t$-test).
...
\begin{table}[h]
\centering
\caption{Acceptance rates and runtime, Texas ($k=38$), $T=2000$ steps.\protect\footnotemark}
\label{tab:accept}
\begin{tabular}{lllll}
\toprule
Method
...
\footnotetext{Runtimes measured on an Intel Core i7-11800H (2.3 GHz, 24 MB L3 cache, Windows 11) using the \texttt{redist-cli} release build (Rust 1.78, \texttt{--release} flag, 8 Rayon threads). Results may differ on different hardware; runtime is dominated by Wilson's spanning tree algorithm, which scales as $O(n \log n)$ on planar graphs.}
