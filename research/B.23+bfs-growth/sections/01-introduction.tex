...
\paragraph{Role 2: Compactness baseline (geometric floor).}
Algorithmic comparisons require a lower bound: what compactness do you
get for free, without any optimisation?
\bfsg{} answers this question.
Its Polsby-Popper scores establish the \emph{geometric floor}---the
compactness achievable by pure proximity-based packing.
Every algorithm that outperforms \bfsg{} on compactness earns its
improvement through deliberate optimisation; every algorithm that
underperforms it has failed to extract even the baseline geographic signal.
...
refinement?
The answer---that it achieves lower compactness than METIS or CVD but
faster than both on a per-node basis---defines its operational envelope.
...
mean Polsby-Popper scores 8--15\% below baseline \metis{}
(Section~\ref{sec:comparison}).
This establishes the geometric floor: METIS's KL refinement alone adds
8--15\% compactness improvement over the bare BFS baseline.
