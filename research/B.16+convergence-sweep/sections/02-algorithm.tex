...
Furthermore, \textbf{METIS must be run single-threaded} (or with a fixed,
explicitly pinned thread count) for the statutory certificate to hold.
OpenMP-parallel METIS uses a non-deterministic work-stealing scheduler:
two runs with the same seed but different thread-scheduling outcomes may
produce different intermediate coarsenings and therefore different local optima.
The \dia{} statutory text specifies that the certified run uses
\texttt{METIS\_OPTION\_NTHREADS=1} (single-threaded mode); independent verifiers
