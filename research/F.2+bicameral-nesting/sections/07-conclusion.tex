...
\begin{enumerate}
  \item \textbf{Compatibility}: 42 of 49 bicameral states have
    $\gcd(H,S) > 1$ and are compatible with NestSection. The remaining
    7 are incompatible due to coprime chamber sizes; small reforms to one
    chamber's size would achieve compatibility in all cases.
  ...
  \item \textbf{Population balance}: All 42 compatible states achieve
...
commissions in the 42 compatible states. The algorithm is implemented in the
...
The next step is to apply NestSection to the 7 incompatible states using
