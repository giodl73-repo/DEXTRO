...
The schedule $\gamma_t = \gamma_0 / t^{3/4}$ satisfies~\eqref{eq:rm-conditions}:
\begin{align*}
  \sum_{t=1}^\infty \frac{\gamma_0}{t^{3/4}} &= \infty
  \quad \text{(divergent $p$-series, $p = 3/4 < 1$)}, \\
  \sum_{t=1}^\infty \frac{\gamma_0^2}{t^{3/2}} &< \infty
  \quad \text{(convergent $p$-series, $p = 3/2 > 1$)}.
\end{align*}
We use the $t^{-3/4}$ schedule rather than the more common $t^{-1}$ (which
converges faster but requires more hand-tuning of $\gamma_0$) or $t^{-1/2}$
(which violates the second Robbins--Monro condition, since
$\sum \gamma_0^2/t$ is the divergent harmonic series).
The $t^{-3/4}$ schedule balances adaptation speed and convergence.
