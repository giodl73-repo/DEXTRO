"""
Run true recursive bisection for 2010 data and save tree structure for hierarchical analysis.
"""
import sys
...
DATA_DIR = BASE_DIR / "data_2010"
RESULTS_DIR = BASE_DIR / "results"
TREE_DIR = BASE_DIR / "trees"
TREE_DIR.mkdir(exist_ok=True)

def load_edge_weights(state: str, year: int = 2010) -> dict:
...
def run_recursive_bisection_with_tree(state: str, n_districts: int):
    ...
    print(f"\n{'='*60}")
    print(f"Processing {state.upper()} (2010) - {n_districts} districts")
    print(f"{'='*60}")
    ...
    # Load edge weights
    edge_weights = load_edge_weights(state, 2010)
    ...
    # Save tree structure
    tree_file = TREE_DIR / f"{state}_2010_tree.pkl"
    ...
    output_file = RESULTS_DIR / f"{state}_2010_true_recursive_partition.csv"
