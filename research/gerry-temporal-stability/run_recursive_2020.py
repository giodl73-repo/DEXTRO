"""
Run recursive bisection partitioning for 2020 census data.

Uses recursive bisection with edge weighting for temporal stability analysis.
"""
...
# District counts for 2020 (same as 2010)
STATE_DISTRICTS = {
    'AL': 7,
    'GA': 14,
    'LA': 6,
...
def run_recursive_partition(state_code: str, year: str = '2020'):
    """Run recursive bisection partitioning for one state."""
    state_name = STATE_TO_NAME[state_code]
    n_districts = STATE_DISTRICTS[state_code]

    print(f"\n{'='*70}")
    print(f"RECURSIVE BISECTION: {state_name.upper()} ({year})")
    print(f"{'='*70}")
    ...
    # Run recursive bisection partitioning (recursive=True)
    print(f"\nRunning METIS recursive bisection to {n_districts} districts...")
    print(f"  Mode: recursive bisection (recursive=True)")
    ...
    partition = partition_graph(
        ...
        nparts=n_districts,
        target_weights=None,
        recursive=True,  # RECURSIVE BISECTION MODE!
    ...
    output_file = output_dir / f'{state_name}_{year}_recursive_partition.csv'
...
def main():
    """Run recursive bisection partitioning for all states."""
    print("="*70)
    print("2020 RECURSIVE BISECTION FOR TEMPORAL STABILITY ANALYSIS")
    print("="*70)
    ...
    for state_code in STATES:
        try:
            elapsed = run_recursive_partition(state_code, '2020')
