...
The algorithm operates without knowledge of racial or ethnic demographics. Nevertheless, districts exhibit measurable demographic characteristics when census demographic data is overlaid post-hoc.

Analyzing the 435 districts reveals: 354 districts (81.4\%) have White majorities, while 81 districts (18.6\%) have non-White majorities (62 Hispanic, 14 Black, 4 Asian, 1 Other). An additional 69 districts have White pluralities but no racial majority.

Current congressional maps contain approximately 100--110 minority opportunity districts, many created intentionally to comply with Voting Rights Act requirements for states with histories of racial discrimination. Our algorithm produces 81 non-White majority districts—fewer than existing plans.

This difference highlights a fundamental limitation: geographic optimization without explicit demographic constraints cannot guarantee VRA compliance. States with VRA obligations would require algorithm augmentation to incorporate racial data as explicit constraints—a technically feasible modification that preserves algorithmic objectivity while satisfying legal requirements.

Duchin \cite{duchin2018gerrymandering} demonstrates that compactness-optimizing algorithms can be constrained to produce specific numbers of majority-minority districts while maintaining objectivity in all other boundary decisions. This "constrained optimization" approach would enable VRA compliance without sacrificing the impossibility defense for partisan gerrymandering.
