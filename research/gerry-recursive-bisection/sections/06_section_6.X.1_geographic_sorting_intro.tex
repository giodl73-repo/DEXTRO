...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/figure_6x1_sorting_vs_bias.pdf}
\caption{Geographic Sorting Predicts Partisan Bias. Each point represents one state, colored by region. The negative correlation ($r \approx -0.7$) shows that states with higher geographic sorting (Democrats concentrated in dense urban areas) systematically produce Republican-leaning outcomes under neutral algorithmic redistricting. Wisconsin (WI), Maryland (MD), and Massachusetts (MA) exemplify extreme geographic sorting with opposite partisan effects depending on state-level demographics. Based on Chen-Rodden (2013) geographic sorting indices and algorithmic redistricting results (2020 Census).}
