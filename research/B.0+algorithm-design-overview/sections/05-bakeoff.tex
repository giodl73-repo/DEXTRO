...
Values without superscript are \emph{confirmed empirical results} from B.8--B.15
...
in B.8--B.9, (b) interpolation from confirmed values in adjacent configurations,
...
Values marked \pendinging{} ($\ddagger$) are \emph{pending}: they require the B.11
(ApportionRegions) or B.12 (ProportionalSection) implementations to run; they should
...
\bottomrule
\multicolumn{5}{l}{\est\ = estimated from B-series models; \pending\ = pending B.11/B.12 implementation.}
\end{tabular}
\end{table}
