...
\begin{lemma}[Isoperimetric scaling]
\label{lem:isoperimetric}
For a uniformly populated convex planar region of total boundary length $L$,
...
By the isoperimetric inequality for convex bodies, the minimum-perimeter enclosure
of area fraction $f$ of a convex region has perimeter proportional to $\sqrt{f}$
for small $f$ (a disk of area $f \cdot A$).
For $f$ near $1/2$, the optimal cut approaches a straight line across the
region's minor axis, with length proportional to $\sqrt{\min(f, 1-f)}$ by
similar scaling.
The approximation holds in the census tract setting when population density
is approximately uniform within the state.
\end{proof}

\begin{remark}
Lemma~\ref{lem:isoperimetric} is an approximation, not an exact result.
Real states have highly non-uniform population density.
The isoperimetric correction remains meaningful in the non-uniform case:
$\sqrt{\min(i,k-i)}$ lower-bounds the expected edge-cut scaling for
a $i:(k-i)$ split in any convex region, so dividing by it makes the
normalised metric more conservative (harder to win) for small-$i$ splits.
