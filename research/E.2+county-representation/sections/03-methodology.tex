...
\subsection{Voting Rights Act Analysis Methodology}

To assess VRA compatibility, we estimate majority-minority district potential under three scenarios: current enacted districts, county-based representation without VRA weighting, and county-based representation with VRA weighting.

\textbf{Majority-Minority District Identification}. We define majority-minority districts as those where combined minority population (Black, Latino, Asian, Native American, Other) exceeds 50\% of total population. Using 2020 enacted district demographics, we count current majority-minority districts nationally and within qualifying counties specifically.

\textbf{Proportional Estimation Approach}. For county-based scenarios, we estimate potential majority-minority districts using proportional allocation: if a county is $X$\% minority and receives $N$ seats, it could create approximately $\lfloor X \times N \rfloor$ majority-minority districts through internal redistricting. This is a \emph{conservative} estimate for two reasons. First, it assumes proportional rather than optimized allocation (actual VRA-compliant redistricting could concentrate minority populations more effectively). Second, it does not account for geographic clustering---minority populations in large urban counties are often spatially concentrated, allowing compact majority-minority districts even when overall percentages are moderate.

\textbf{VRA Weighting Parameters}. For the VRA-aware scenario, we assume counties would use recursive bisection with minority population thresholds. Following established redistricting research parameters: census tracts with 40\% or greater minority population receive 5$\times$ edge weighting during graph partitioning. This promotes geographic concentration of minority populations, creating compact majority-minority districts where demographic conditions warrant. This approach has been validated in academic redistricting literature and reflects standard VRA compliance techniques.

\textbf{Methodological Justification}. Reviewers may question using estimation rather than actual county-internal redistricting. We justify this approach on four grounds. First, proportional estimation is standard for VRA pre-clearance analysis---states routinely use such estimates when planning redistricting to assess compliance. Second, our estimates are conservative; actual optimized redistricting would likely produce more majority-minority districts. Third, running full VRA-compliant recursive bisection for all 25 counties would require substantial computational resources and tract-level demographic data for county-internal boundaries, which is implementation work beyond the scope of this normative analysis. Fourth, the magnitude of the difference (68 current vs. 197-201 county-based) is so substantial that even significant estimation error would not change the qualitative conclusion that county-based representation dramatically improves minority representation. Future work should validate these estimates through actual county-internal redistricting, but for establishing feasibility and demonstrating VRA compatibility, proportional estimation provides sufficient evidence.
