...
\[
  \mathrm{EC}_{\min}(G) \geq \frac{\lambda_2 \cdot W_{\mathrm{total}}}{4}
\]
where $W_{\mathrm{total}} = \sum_{(i,j)\in E} w_{ij}$ is the total edge weight.
...
\label{prop:immunity}
Any party challenging a Fiedler-certified plan on the grounds that a more
compact plan exists must exhibit a bisection of the certified subgraph with
geometric-mean compactness exceeding
$(1-\delta) \cdot \overline{\mathrm{GMPP}}(G)$
on the certified level.
Since $\overline{\mathrm{GMPP}}(G)$ is a mathematical upper bound on any
bisection, no such challenge can succeed when $\delta = 0$; for $\delta > 0$,
the challenger must improve by more than $\delta \times 100$\% --- which
requires running at least as many seeds as the certifier did.
\end{proposition}
