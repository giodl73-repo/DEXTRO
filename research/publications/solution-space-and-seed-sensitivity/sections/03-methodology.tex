...
\subsubsection{Computation of $\lambda_2$}

For US census-tract graphs with $n \approx 200$--$4{,}000$ vertices,
$\lambda_2$ can be computed exactly via the Lanczos
algorithm~\citep{golub2013matrix} in $O(n^2)$ time --- milliseconds per
subgraph. The computation uses only the graph adjacency structure and TIGER
boundary lengths, both of which are in the published reproducibility artifacts.
Any party can verify the reported $\lambda_2$ independently using publicly
available linear algebra libraries.

The certificate is therefore fully reproducible: $\lambda_2$ is deterministic
given the graph; the upper bound $\overline{\mathrm{GMPP}}$ follows
algebraically; the achieved ratio is computed from the published split. No
proprietary data or computation enters the certificate.
