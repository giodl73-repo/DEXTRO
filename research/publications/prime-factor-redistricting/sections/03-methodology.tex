...
\begin{theorem}[Reuse]
  ...
  $\text{PFR}(G, n, \sigma)$ and $\text{PFR}(G, n', \sigma)$ begin with the
...
\begin{corollary}
  $\text{PFR}(G, 34, \sigma)$ and $\text{PFR}(G, 51, \sigma)$ begin with
...
\emph{PFR-binary}. PFR-binary is formally distinct from direct-$k$-METIS:
...
theorem holds for PFR-binary under the same determinism assumption.

A variant \emph{PFR-direct} that uses direct $k$-way METIS for all prime $k$
is fully equivalent to PFR-binary in our implementation: METIS~5.x's
\texttt{part\_kway} handles $k = 17$ directly (single call, 0.1\% tolerance).
We defer a systematic comparison of PFR-binary vs.\ PFR-direct to future work.
