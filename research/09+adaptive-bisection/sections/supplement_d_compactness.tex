% Supplement D: Compactness Metrics and Enacted Plans Comparison
% Extended analysis of geometric properties and comparison to 2020 maps

Beyond VRA compliance (minority-majority district counts), we analyze compactness to assess whether method equivalence extends to geometric properties. We also compare algorithmic plans to enacted 2020 redistricting plans.
...
\subsection{Compactness Equivalence Across Methods}

Table~\ref{tab:compactness-comparison-supp} shows compactness scores for our algorithmic methods:
...
\label{tab:compactness-comparison-supp}
\end{table}
...
\textbf{Interpretation}: Method equivalence extends beyond VRA metrics (max minority \%, MM counts) to geometric properties (compactness). All algorithms produce the same partition, which inherently has the same compactness.

\subsection{Comparison to Enacted 2020 Plans}

Table~\ref{tab:enacted-comparison-supp} compares our algorithmic plans to enacted 2020 congressional maps:
...
\bottomrule
\end{tabular}
\caption{Comparison of algorithmic edge-weighted plans (all three methods identical) to enacted 2020 congressional maps. Algorithmic plans produce 75\% more MM districts (14 vs. 8) while maintaining higher compactness (mean PP: 0.41 vs. 0.38). This suggests enacted plans sacrifice both VRA compliance and compactness, potentially due to partisan gerrymandering or incumbent protection.}
\label{tab:enacted-comparison-supp}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Mechanism}: Enacted plans likely sacrifice compactness to achieve partisan goals (packing/cracking). Algorithmic plans optimize edge-cut, which naturally produces compact districts.
\end{enumerate}

\subsection{Compactness vs. VRA Compliance Trade-off}

Traditional redistricting assumes a trade-off: creating MM districts requires sacrificing compactness (``packing'' minorities into irregular districts). Our results refute this assumption by demonstrating algorithmic plans achieve higher VRA compliance and higher compactness simultaneously.

\textbf{Explanation}: Edge-weighting creates an \textit{aligned objective}: minimizing edge-cut naturally:
\begin{enumerate}
...
\begin{itemize}
    \item \textbf{Gingles prong 1}: Plaintiffs must show minority communities are ``sufficiently large and geographically compact'' to constitute a majority in a district~\cite{gingles1986}. Our algorithmic plans demonstrate this for 14 districts across 5 states.
    ...
    \item \textbf{Narrow tailoring}: If enacted plans claim to balance VRA compliance with compactness, our results show this trade-off is false. Algorithmic plans achieve both goals simultaneously.
...
\subsection{Alternative Compactness Metrics}
...
\label{tab:alternative-compactness-supp}
\end{table}
...
Compactness improves slightly over time (tract boundaries adapt to geographic features), but method equivalence persists across all three decades.

\subsection{Summary}

Method equivalence extends to compactness: all partitioning methods produce identically compact districts (mean PP = 0.41 $\pm$ 0.001) when using edge-weighting with $\alpha = 5$. This refutes the traditional VRA-compactness trade-off.
