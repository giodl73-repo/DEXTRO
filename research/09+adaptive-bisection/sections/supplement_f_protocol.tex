% Supplement F: Experimental Protocol Details
...
\begin{itemize}
    ...
    \item Adjacency is symmetric: if tract $i$ is adjacent to tract $j$, then tract $j$ is adjacent to tract $i$
...
\begin{enumerate}
    ...
    \item Range of district counts ($k \in \{4, 6, 7, 14\}$) to test scalability
...
\begin{enumerate}
    ...
    \item \textbf{Adaptive Recursive Bisection}: Same as predetermined but evaluates all $k$ candidate first splits and selects the one maximizing minority concentration. (In our experiments, adaptive always selected $(1, k-1)$ due to edge-weighting.)
...
\begin{itemize}
    \item Robustness test: 10 random seeds for each method times each state = 150 runs
...
\begin{itemize}
    \item Fixed seed: \texttt{-seed=42} for all runs in main experiment
    \item Multiple seeds: \texttt{-seed=\{1,2,3,...,10\}} for robustness test
...
For the main experiment, we report results from a \textit{single run} per method per state using \texttt{-seed=42}. This is justified because:
\begin{enumerate}
    \item Edge-weighting with $\alpha = 5$ induces deterministic behavior
    \item Robustness test shows variance across seeds is negligible ($< 0.1\%$)
...
\begin{itemize}
    ...
    \item \textbf{Runtime}: Wall-clock time from start of METIS call to return, measured using Python \texttt{time.perf\_counter()}
\end{itemize}
...
\begin{enumerate}
    ...
    \item Determinism is theoretical, not empirical statistical inference
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Run \texttt{build\_adjacency.py} to construct graphs
    ...
    \item Run \texttt{analyze\_results.py} to compute metrics
    \item Run \texttt{create\_visualizations.py} to generate figures
\end{enumerate}
