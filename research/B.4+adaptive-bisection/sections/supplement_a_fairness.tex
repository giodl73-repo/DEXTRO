% Supplement A: Algorithmic Fairness Framework
% Extended discussion of fairness theory and redistricting implications

Beyond the operational considerations of computational efficiency and VRA compliance, our finding connects to fundamental questions in algorithmic fairness~\cite{dwork2012fairness,hardt2016equality}. When multiple algorithms can produce different outcomes for the same task, a natural question arises: which outcome is ``fair''?

\subsection{Individual Fairness and Algorithmic Determinism}
...
Algorithmic choice dependence creates vulnerability to strategic manipulation. If practitioners know that method $\mathcal{A}_1$ favors their party while method $\mathcal{A}_2$ does not, they can ``game'' the system by choosing $\mathcal{A}_1$ while claiming neutrality (``we used a standard algorithm'').
...
\subsection{Fairness-Transparency Trade-off}
...
\subsection{Connection to Procedural Fairness}
...
\begin{enumerate}
    ...
    \item \textbf{Transparency}: Process should be understandable to stakeholders
    \item \textbf{Correctability}: Errors should be detectable and fixable
\end{enumerate}

Method independence satisfies all three:
\begin{itemize}
    \item Consistency holds because all algorithms converge to the same partition
...
\begin{itemize}
    ...
    \item \textbf{Enhances legitimacy}: Outcomes are invariant to algorithmic details
...
These properties align with calls for algorithmic accountability in high-stakes decision-making~\cite{kroll2017accountable,diakopoulos2016accountability}. We expand on these fairness guarantees in Appendix E.
